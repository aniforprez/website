...
<InsertThought>
  I have to admit, removing the older symlinks and the files from my dotbot repo
  gave me a *strong* sense of satisfaction haha!
</InsertThought>
...
It is _highly_ recommended that all scripts that you add here are idempotent i.e. after you run the scripts once, they should always return the same result. If all the packages are installed already, they should just not do anything. This means there should be no side effects to any of your scripts so it is best to strictly avoid things like copying files to relative paths and such.
...
With this simple config change, chezmoi will autocommit the repo with a commit message stating the name of the file that was changed and automatically pushing to the remote repository. Some people might think this is dangerous but I highly doubt I'm making any permanently breaking changes when I make changes in the chezmoi directory. At worst I accidentally delete a file. _It'll be fine_.
