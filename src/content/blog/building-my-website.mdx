...
pubDate: "2025-06-11"
heroImage: "../../assets/images/building-my-website.jpg"
...
The contact form on the home page is a simple form submitted using ajax and captured by Netlify using [their form service](https://docs.netlify.com/forms/setup/#submit-html-forms-with-ajax) (especially easy since the website is [deployed on Netlify](#publication)). I decided to add tooltips here and there to show descriptions and such implemented with [TippyJS](https://atomiks.github.io/tippyjs/) and found icons on [Remix Icon](https://remixicon.com/) that sort of fit the vibes I wanted.

### About Me

This took some time to come together, mostly due to inertia and some level of anxiety when working on it, but ultimately I settled on having a good amount of text describing myself, a nice photo, some links, and my technial work history. The first three were were achieved in an uncomplicated manner but for my work history I decided to use [JSON Resume](https://jsonresume.org/) which is a spec that provides a schema to detail out your technical experience using JSON which can also be used to generate PDFs. All I had to do was import the `resume.json` file and iterate through it in the template to generate the HTML.
...
[^4]: [Fix for events when using View Transitions by Cristian Perez Rodriguez](https://www.cpr.name/blog/astro-event-handling)
